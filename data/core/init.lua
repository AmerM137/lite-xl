...
local function save_session()
	...
	if fp then
		fp:write(
			...
			", previous_find=",
			common.serialize(core.previous_find),
			", previous_replace=",
			common.serialize(core.previous_replace),
			"}\n"
		)
		fp:close()
	end
...
local function show_max_files_warning(dir)
	local message = dir.slow_filesystem and "Filesystem is too slow: project files will not be indexed."
		or "Too many files in project directory: stopped reading at "
...
local function refresh_directory(topdir, target)
	...
	local files = dirwatch.get_directory_files(topdir, topdir.name, (target or ""), 0, function()
		return false
	end)
	local change = false
	...
	while old_idx <= directory_end_idx or new_idx <= #files do
		...
		if not files_info_equal(new_info, old_info) then
			...
			if
				not old_info
				or (
					new_info and system.path_compare(new_info.filename, new_info.type, old_info.filename, old_info.type)
				)
			then
			...
			local size = old_info
					and old_info.type == "dir"
					and select(2, project_subdir_bounds(topdir, old_info.filename, old_idx))
				or 1
...
function core.add_project_directory(path)
	...
	if not complete then
		...
		for i, v in ipairs(t) do
			if v.type == "dir" then
				topdir.watch:watch(path .. PATHSEP .. v.filename)
			end
		end
	end
	...
	topdir.watch_thread = core.add_thread(function()
		while true do
			local changed = topdir.watch:check(function(target)
				if target == topdir.name then
					return refresh_directory(topdir)
				end
				...
				if dirpath then
					...
					if not dir_match or not core.project_subdir_is_shown(topdir, topdir.files[dir_index].filename) then
						return
					end
				end
...
function core.rescan_project_directories()
	...
	for i = 1, n do -- add again the directories in the project
		...
		if dir.files_limit then
			...
			table.sort(subdir_list, function(a, b)
				return system.path_compare(a, "dir", b, "dir")
			end)
...
function core.project_files_number()
	local n = 0
	for i = 1, #core.project_directories do
		if core.project_directories[i].files_limit then
			return
		end
...
local function write_user_init_file(init_filename)
	...
	if not init_file then
		error('cannot create file: "' .. init_filename .. '"')
	end
	init_file:write([[
...
function core.init()
	...
	if set_project_ok then
		...
		if
			not core.set_project_dir(project_dir_abs, function()
				got_project_error = not core.load_project_module()
			end)
		then
		...
		for _, entry in pairs(plugins_refuse_list) do
			if #entry.plugins > 0 then
				msg[#msg + 1] = string.format(
					...
					common.home_encode(entry.dir),
					table.concat(entry.plugins, "\n")
				)
			end
		end
		core.nag_view:show(
...
function core.temp_filename(ext, dir)
	...
	return dir .. PATHSEP .. temp_file_prefix .. string.format("%06x", temp_file_counter) .. (ext or "")
end
...
local function get_plugin_details(filename)
	...
	if not info or not filename:match("%.lua$") then
		return false
	end
	...
	for _, root_dir in ipairs({ DATADIR, USERDIR }) do
		...
		for _, filename in ipairs(system.list_dir(plugin_dir) or {}) do
			if not files[filename] then
				table.insert(ordered, { file = filename })
			end
	...
	for _, plugin in ipairs(ordered) do
		if plugin.valid then
			if not config.skip_plugins_version and not plugin.version_match then
				core.log_quiet("Version mismatch for plugin %q from %s", plugin.name, plugin.dir)
				local rlist = plugin.dir:find(USERDIR, 1, true) == 1 and "userdir" or "datadir"
...
function core.add_thread(f, weak_ref, ...)
	...
	local args = { ... }
	local fn = function()
		return core.try(f, table.unpack(args))
	end
...
function core.step()
	...
	if current_title ~= nil and current_title ~= core.window_title then
		system.set_window_title(core.project_dir:match("[^/\\]+$") or "Lite XL")
